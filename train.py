...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--name", default="DaB", help="Name of the run")
    parser.add_argument("--cuda", default=False, action="store_true", help="Enable CUDA")
    args = parser.parse_args()
    device = torch.device("cuda" if args.cuda else "cpu")

    saves_path = os.path.join(
        "/media/Usuario/Documentos/Documentos/AAMaestría/2. Segundo semestre/Aprendizaje Reforzado/DaBtrain", args.name)
    os.makedirs(saves_path, exist_ok=True)
    writer = SummaryWriter(comment="-" + args.name)

    net = model.Net(input_shape=model.OBS_SHAPE, actions_n=game.EDGES).to(device)
