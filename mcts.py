...
class MCTS:
    ...
    def find_leaf(self, state_int, player):
        ...
        while not self.is_leaf(cur_state):
            ...
            invalid_actions = set(range(game.EDGES)) - \
                              set(game.possible_moves(cur_state))
            for invalid in invalid_actions:
                score[invalid] = -np.inf
            action = int(np.argmax(score))
            actions.append(action)
            cur_state, won, next_player = game.move(
    ...
    def search_minibatch(self, count, state_int, player,
        ...
        if expand_queue:
            ...
            for (leaf_state, states, actions), value, prob in \
                ...
                self.visit_count[leaf_state] = [0]*game.EDGES
                self.value[leaf_state] = [0.0]*game.EDGES
                self.value_avg[leaf_state] = [0.0]*game.EDGES
